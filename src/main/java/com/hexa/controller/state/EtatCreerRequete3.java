	...
	public void clicGauche(Controller c, Window w, Coordonnees coordonneesSouris, ListOfCommands l) {
		...
		for (Intersection intersection : intersectionsEtEntrepot) {
			...
			if (coord.equals(coordonneesSouris)
					&& (c.getTournee().estLieuLivraison(intersection) || entrepot.equals(intersection))) {
				c.getEtatCreerRequete2().entryAction(this.intersectionAjouter, intersection);
				c.setCurrentState(c.getEtatCreerRequete2());
				w.afficherMessage("Intersection sélectionnée pour la livraison : " + intersectionAjouter.toString()
