...
public class Tournee extends Observable {
	...
	/**
	 * Livreur chargé d'effectuer la tournée
	 */
	...
	public boolean supprimerLivraisonApresCalcul(Livraison livraison, Graphe carte) throws TourneeException {
		if (!circuitCalculer || !livraisons.contains(livraison)) {
			return false;
		}
	 ...
	 * @param carte
	 */
	public boolean genererFeuilleDeRoute(Graphe carte) {
		if (!circuitCalculer) {
			return false;
		}
		...
		String nomFichier = "Feuille_de_route_livreur_" + livreur.getId() + "_"
		...
		while (circuit.hasNext()) {
			...
			if ((livraison = getLivraison(segment.getDestination())) != null) {
				text += "Livraison à " + nomSegment + " : heure estimée d'arrivée: " + livraison.getHeureEstime()[0]
						+ "h" + livraison.getHeureEstime()[1];
				if (livraison.getNbMinutesAttente() != 0) {
					text += " et attendez " + livraison.getNbMinutesAttente() + " minutes car";
				}
				text += " la plage horaire est : " + livraison.getPlageHoraire()[0] + "h->"
						+ livraison.getPlageHoraire()[1] + "h\n";
			}
	...
	public boolean equals(Tournee t) {
		return this.livreur.getId() == t.getLivreur().getId();
	}
